...
object Classifier {
  ...
  def props(sparkContext: SparkContext, twitterHandler: ActorRef, onlineTrainer: ActorRef, predictor: PredictorProxy) =
    Props(new Classifier(sparkContext, twitterHandler, onlineTrainer, predictor))

  case class Classify(keyword: String, oAuthKeys: OAuthKeys)

  case class Point(tweet: String, tokens: Seq[String])

  case class ClassificationResult(onlineModelResult: Seq[LabeledTweet])
...
class Classifier(sparkContext: SparkContext, twitterHandler: ActorRef, onlineTrainer: ActorRef, predictor: PredictorProxy) extends Actor with ActorLogging {
  ...
  override def receive = LoggingReceive {
    ...
    case Classify(keyword: String, oAuthKeys: OAuthKeys) =>
      ...
      val handler = context.actorOf(FetchResponseHandler.props(onlineTrainer, originalSender, sparkContext, predictor), "fetch-response-message-handler")
...
object FetchResponseHandler {
  ...
  def props(onlineTrainer: ActorRef, originalSender: ActorRef, sparkContext: SparkContext, predictor: PredictorProxy) =
    Props(new FetchResponseHandler(onlineTrainer, originalSender, sparkContext, predictor))
}

class FetchResponseHandler(onlineTrainer: ActorRef, originalSender: ActorRef, sparkContext: SparkContext, predictor: PredictorProxy) extends Actor with ActorLogging {
...
class TrainingModelResponseHandler(fetchResponse: FetchResponse, originalSender: ActorRef, sparkContext: SparkContext, predictor: PredictorProxy) extends Actor with ActorLogging {
  ...
  def predict = (onlineFeatures, batchTrainerModel, onlineTrainerModel) match {
    ...
    case (Some(onlineF), Some(batchM), Some(onlineM)) =>
      ...
      sendResponseAndShutdown(ClassificationResult(onlineModelResult))
